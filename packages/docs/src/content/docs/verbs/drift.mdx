...
> **v1.1.0 — auto-audit.** Run `basalt audit --drift-v1` to re-evaluate
> previously surfaced Drift findings against the current 30-day window.
> Each finding is tagged with `confirmed` / `softened` / `reversed` /
> `vanished`. No LLM involved — purely numerical.
